...
Dim uname,title,pwd,pwd2,contents,reg_ip,mod_ip
Dim attachDel, dbFileSaveName
Dim saveFile
...
idx = req(theForm("idx")(1))
Page = req(theForm("Page")(1))
SearchOpt = req(theForm("SearchOpt")(1))
SearchVal = req(theForm("SearchVal")(1))
...
attachDel = req(theForm("attachDel")(1))

if Split(referer,"?")(0)<>"http://localhost/mod_upload.asp" then 
 ...
 <head>
  ...
  <script language="javascript" type="text/javascript">
	$(document).ready(function(){
		...
		setTimeout(function(){
			location.href = "view_upload.asp?idx="+$idx+"&Page="+$Page+"&SearchOpt="+$SearchOpt+"&SearchVal="+$SearchVal;
		},5000);
	});
  </script>
 </head>
	<body>
		...
		if res=0 then
			...
			'첨부삭제 체크시 처리
			'Response.Write "attachDel : " & attachDel & "<br>"
			if not isnull(attachDel) and not isempty(attachDel) and attachDel<>"" then
				
				Set fso = CreateObject("Scripting.FileSystemObject")
				'Response.Write Ubound(attachDel)
				if Instr(attachDel,",")>0 then
					attachDel = Split(attachDel,",")
					for i=0 to Ubound(attachDel) step 1
						err.Clear
						On error resume next
							saveFile = Server.MapPath( "./upload/" & trim(attachDel(i)) )
							If fso.FileExists(saveFile) Then
								fso.DeleteFile(saveFile)
							End if

							dbFileSaveName = trim(Split(attachDel(i),"/")(1))
							'Response.Write "dbFileSaveName1 : " & dbFileSaveName & "<br>"
							Set oCmd = Server.CreateObject("ADODB.Command")
							With oCmd
								.ActiveConnection = oConn
								.CommandType = adCmdStoredProc
								.CommandText = "dbo.sp_board_upload_del_ok"
								
								.Parameters.Append .CreateParameter("@fileSaveName",adVarWChar,adParamInput,50,dbFileSaveName)
								.Parameters.Append .CreateParameter("@res",adSmallInt,adParamOutput,0,0)

								.Execute , , adExecuteNoRecords

								res = .Parameters("@res") '0:성공 1:sql error
							End With
							SetFreeObj(oCmd)
						if err.number<>0 then
					...
					On error resume next
						If fso.FileExists(saveFile) Then
							saveFile = Server.MapPath( "./upload/" & trim(attachDel) )
							fso.DeleteFile(saveFile)
						End if
						'Response.Write "saveFile : " & saveFile & "<br>"
						
						dbFileSaveName = trim(Split(attachDel,"/")(1))
						'Response.Write "dbFileSaveName2 : " & dbFileSaveName & "<br>"
						Set oCmd = Server.CreateObject("ADODB.Command")
						With oCmd
							.ActiveConnection = oConn
							.CommandType = adCmdStoredProc
				...
				if trim(fileRealNames(i))<>"" then
					...
					On error Resume next
						...
						With oCmd
							...
							.Parameters.Append .CreateParameter("@bidx",adInteger,adParamInput,0,idx)
