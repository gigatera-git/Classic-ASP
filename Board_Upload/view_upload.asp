 ...
 <head>
  ...
  $(document).ready(function(){
		...
		$("#btnReply").on("click",function(){
			location.href = "reply_upload.asp?idx="+$idx+"&Page="+$Page+"&SearchOpt="+$SearchOpt+"&SearchVal="+$SearchVal+"&ref="+$ref+"&re_step="+$re_step+"&re_lvl="+$re_lvl;
		});
		...
		$("#btnMod").on("click",function(){
			location.href = "mod_upload.asp?idx="+$idx+"&Page="+$Page+"&SearchOpt="+$SearchOpt+"&SearchVal="+$SearchVal;
		});
		...
		$("#btnPwdChkOk").on("click",function(e){
			...
			if ($res!='ok') {
				...
				alert("비밀번호가 확인되었습니다")
				$("#pwd").val($("#pwdChk").val());
				$("#frmBoard").attr({'action':'del_upload_ok.asp','method':'post'}).submit();
			}
		});
	...
	<form name="frmBoard" id="frmBoard">
		...
		if not ors.eof and not ors.bof then
			...
			while not ors.eof and not ors.bof 
				...
				Response.Write "<input type='hidden' name='files' class='files' value='"& Left(oRs("reg_date"),10) &"/"& trim(oRs("fileSaveName")) &"' />"
				Response.Write "</div>"
				ors.movenext
