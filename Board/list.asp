...
<html lang="ko">
 ...
 <body>
	...
	while not ors.eof and not ors.bof
		...
		<td align="left"><img src="./images/common/level.gif" border="0" align="absmiddle" width="<%=oRs("re_lvl")*7%>"><a href="view.asp?idx=<%=oRs("idx")%>&Page=<%=Page%>&<%=argv%>"><%=oRs("title")%></a></td>
	...
	<div id="search">
		<select name="SearchOpt" id="SearchOpt" required oninvalid="this.setCustomValidity('검색옵션을 선택하세요')" oninput="setCustomValidity('')">
			<option value=""></option>
			<option value="title" <%if (SearchOpt="title" and SearchVal<>"") then response.write " selected" %>>제목</option>
			<option value="contents" <%if (SearchOpt="contents" and SearchVal<>"") then response.write " selected" %>>>내용</option>
		</select>
		<input type="text" name="SearchVal" id="SearchVal" maxlength="10" minlength="2" required oninvalid="this.setCustomValidity('검색어를 입력하세요')" oninput="setCustomValidity('')" value="<%if (SearchOpt<>"" and SearchVal<>"") then Response.Write SearchVal%>">
		<input type="submit" value="검색" id="btnSearch" name="btnSearch">
		<%
		if (SearchOpt<>"" and SearchVal<>"") then
			Response.Write "<input type='button' value='처음' id='btnInit'>";
		end if
		%>
	</div>
	...
	<%
	If Cint(Page)>1 Then
		Response.Write "<a href='list.asp?Page=1&"&argv&"'>[처음]</a>"
	Else
		Response.Write "[처음]"
	End If
	response.write "&nbsp;"
	...
	If intTemp = 1 Then
		Response.Write "[이전]"
	Else 
		Response.Write"<a href='list.asp?Page=" & intTemp - intBlockPage & "&"&argv&"'>[이전]</a>"
	End If
	response.write "&nbsp;"
	%>
	...
	Do Until intLoop > intBlockPage Or intTemp > intTotalPage
		...
		Else
			Response.Write"<span><a href='list.asp?Page=" & intTemp & "&"&argv&"'>"&intTemp&"</a></span>"
		End If
		response.write "&nbsp;"
	...
	If intTemp > intTotalPage Then
		Response.Write "[다음]"
	Else
		Response.Write"<a href='list.asp?Page=" & intTemp & "&"&argv&"'>[다음]</a>"
	End If
	response.write "&nbsp;"
	
	If Cint(Page) < Cint(intTotalPage) Then
		Response.Write "<a href='list.asp?Page=" & intTotalPage & "&"&argv&"'>[마지막]</a> "
	Else
		Response.Write "[마지막]"
	End If
	%>
